...
//    VideoCapture cap("/Users/paulnta/Documents/HEIG4/PDG/Led_Tests/iPhone_light.mov");
    VideoCapture cap(0);
    if(!cap.isOpened())
        return;
    ...
    ld->activateDebugBgSubstraction();
...
cv::Mat frame;
  ...
  cv::namedWindow ("Background", 0);
    ...
    for (;;){
        ...
        Size newSize(H, cvRound((double)H*frame.rows/frame.cols));
        resize(frame, frame, newSize);
        bg->apply(frame,fore, 0.1);
        bg->getBackgroundImage(back);
        cv::erode (fore, fore, cv::Mat ());
        cv::dilate (fore, fore, cv::Mat ());

        Mat contoured;
        fore.copyTo(contoured);
        cv::findContours (contoured, contours, CV_RETR_EXTERNAL,CV_CHAIN_APPROX_NONE);
